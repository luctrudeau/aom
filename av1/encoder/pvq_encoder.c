...
static int pvq_theta(od_coeff *out, const od_coeff *x0, const od_coeff *r0,
  ...
  *max_theta = 0;
  ...
  if (!skip_can_copy) skip_dist = gain_weight*cg*cg*OD_CGAIN_SCALE_2;
  else {
  ...
  if (skip_can_copy) {
    /* if skip_can_copy, noref == 0 and gain == 0 aren't allowed */
  ...
  *skip_diff += skip_dist - best_dist;
  /* Encode gain differently depending on whether we use prediction or not.
     ...
     but not noref=1.*/
  if (skip_can_copy) return noref ? qg - 1 : neg_interleave(qg + 1, icgr + 1);
 ...
 * @param [in]     skip_can_copy whether skip can copy the reference
...
#error "CONFIG_PVQ currently requires CONFIG_DAALA_EC."
  ...
  cfl_encoded = 0;
  skip_rest = 1;
  skip_theta_value = skip_can_copy ? 0 : -1;
  ...
  for (i = 0; i < nb_bands; i++) {
    ...
    if (i == 0 || (!skip_rest && !(skip_dir & (1 << ((i - 1)%3))))) {
      pvq_encode_partition(&enc->w, qg[i], theta[i], max_theta[i], y + off[i],
       ...
       + bs*PVQ_MAX_PARTITIONS + i, skip_can_copy, i == 0 && (i < nb_bands - 1),
...
#endif
    if (skip_can_copy) for (i = 1; i < 1 << (2*bs + 4); i++) out[i] = ref[i];
